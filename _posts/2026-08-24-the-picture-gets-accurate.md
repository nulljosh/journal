...
Pulled metadata for four apps where I dropped China, shipped macOS versions of Charwork and Curvely using APIs ported from Sidewise (also fixed Charwork's build script), gave Cadence the icon it never had, and built an animated math expression wall for Numen's hero. Reshaped Feng Shui into an assessment tool with twenty-four yes/no questions across four energy layers, each answer expanding with book reasoning. Fixed Epiphany's landing map (CSP needed blob: for MapLibre), reskinned the journal site to match the family, and freed twenty-six gigs of Xcode cache. Joshua signed in, finding six bugs in Roost and fixed them with cache mirrors, Esri tiles, thinner pins, Supabase favorites sync, and route gating. Late evening, two approvals landed: Inkpress macOS first release for Mac, and Curvely's appeal overturning the 4.3(a) flag.
