...
Discovered Lexly production was broken: CSP blocked the Supabase SDK from cdn.jsdelivr.net, createClient threw silently, site went blank. Fixed CSP, made auth failures graceful, gated catalog behind auth, reduced cache to 5 minutes, bumped service worker to lexly-v5, deduplicated content (punctuation/case-insensitive), verified in Chrome. Fixed a critical iOS bug where exercises were silently dropping half their fields (audio, word-bank, matching pairs, language tags), making exercises unanswerable; ported hearts and SM-2 to iOS, built a generator for 1,800 new exercises from freely-licensed Tatoeba, and added match/cloze types (content: 830→2,615). Verified new types via jsdom (14/14 assertions on logic and DOM), recorded gaps in roadmap. Patched XSS in Sidewise's feed-title parsing, rolled security headers to nine sites, fixed Talli's package-lock, and merged dotfiles cleanup.
