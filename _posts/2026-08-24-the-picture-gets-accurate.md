...
Pulled canonical App Store metadata from live and dropped China from four apps to dodge 2.1 rejections. Hardened security across ten apps with response headers and standardized landing page CTAs. Charwork got macOS, HTTP API, and MCP entry; added three new Lexly courses (Discrete Math, Reverse Engineering, iOS Jailbreaks). Debugged a five-day SnapTrade outage where the client ID got mangled during the Vercel-to-Cloudflare cutover; fixed it and stopped leaking raw errors to the UI. Built Numen, a free-form calculator on the web with recursive parsing, cycle-safe sheets, draggable cards, and inline graphs; shipped to Cloudflare Pages with tests passing.
