...
Evening wrap: Joshua pushed back on homeward being PWA-only. Built homeward/kmp from the Kotlin Multiplatform template with shared Ktor client and Compose board UI, wired both Android and macOS/Windows desktop targets. Tests pass, debug APK and DMG both build locally. Added a reusable native-release CI workflow to labs (.github/workflows/native-release.yml) to build the real MSI/DEB/APK on Windows/Ubuntu runners for any app with a kmp/ module; triggered it for homeward and it was still building when the wrap started. Rewrote the landing page's install card to promise a real installable app instead of "add to home screen", deployed to Cloudflare. Notes recorded on JDK/vendor checks and unfinished release step (signed APK + keystore).

## Monday (2026-08-31)

Fixed Talli's redirect loop, shipped PWA across twenty apps, and built Swing (random video chat with Durable Object pairing); added TURN support but left it STUN-only since the app has no users yet and minting TURN keys needs a card on file. Closed a critical Homeward security hole where edit tokens leaked from the read policy, fixed by moving them to a create-listing RPC with landing page, icon, and OAuth sign-in. Fixed the journal's CSP style-src blocking color tokens and animations, rebuilt the landing page, patched the same bug on sidewise, repaletted homeward, and fixed found-chip visibility by switching to solid black fills.
