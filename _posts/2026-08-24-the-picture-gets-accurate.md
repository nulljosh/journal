...
Ran Lexly iOS in simulator for the first time: opened on book summaries (CatalogView sorted alphabetically, books invisible on web so iOS-only). This is the real Guideline 2.1 cause, not territory licensing as diagnosed; moved 15 masterclasses to bookrank. Verified on device with AXe: all five exercise types work, hearts decrement. Fixed iOS exercise bug (dropped half their fields); ported hearts/SM-2 to iOS, built 1,800 Tatoeba exercises, added match/cloze (830→2,615). Discovered production was broken (CSP blocked Supabase SDK); fixed CSP, gated catalog, beefed landing page (stats, five types, Tatoeba, FAQ), built demo (/app/?demo=1), added OG/cards, dark mode, WebP (347→79KB).
