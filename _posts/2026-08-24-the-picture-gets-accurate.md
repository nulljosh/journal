...
Discovered Lexly production was broken: CSP blocked Supabase SDK from cdn.jsdelivr.net, site went blank. Fixed CSP, made auth graceful, gated catalog, dropped cache to 5 minutes, bumped service worker, deduplicated content (punctuation/case-insensitive), verified in Chrome. Fixed iOS bug where exercises dropped half their fields (audio/word-bank/pairs/language), ported hearts and SM-2 to iOS, built 1,800 exercises from Tatoeba, added match/cloze (830→2,615). Beefed landing page: stats bar (2,600 exercises, 12 languages, 44 courses), five exercise types with pedagogy, Tatoeba attribution, FAQ; fixed macOS status (was pending, now live). Verified new types via jsdom (14 assertions), recorded gaps, patched XSS in Sidewise, rolled security headers to nine sites, fixed Talli's lock, merged cleanup.
