...
Pulled canonical App Store metadata from live and dropped China from four apps to dodge 2.1 rejections. Hardened security across ten apps with response headers and standardized landing page CTAs. Charwork and Curvely both shipped macOS versions with shared HTTP APIs and MCP servers ported from Sidewise, fixing platform-specific entitlements, plist placement, and icon sizing. All apps in the estate now have macOS targets. Built Numen, a free-form calculator: Pages custom-domain attachment failed on both tokens, so deployed as a Cloudflare Worker with static assets, landing page at /, app at /app, and added to portfolio.
