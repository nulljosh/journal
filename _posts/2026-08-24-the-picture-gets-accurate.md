...
Got Lexly running on iOS: the catalog was breaking because it had been invisible on web, so I moved the masterclasses to bookrank and fixed it. Reworked units from keywords instead of tiers, added lesson cards with grammar notes across five languages, and loosened the filter to boost smaller languages. Audited the whole portfolio (32 apps, 12 live) and found three blockers: Form 506 gates iOS revenue (requires mailed+faxed request, not online), no analytics dashboards exist, and no metadata is checked in. Repriced Voxprint's IAP to $9.99 and swept support/marketing URLs across all apps: fixed Healstack and Voxprint iOS in ASC, found Talli's support still dead since the rename and others frozen until their next version bump. Hypothesis: the 4.3(a) spam wave may be self-inflicted by apps sharing one generic URL.
