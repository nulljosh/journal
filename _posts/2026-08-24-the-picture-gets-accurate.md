...
Audited sign-in across all the apps and discovered a long-standing Chrome automation blocker was actually just an unpaired browser session. Fixed that, then registered the Google OAuth client for the shared Supabase project: client ID is ready and a new secret was minted since Google quietly stopped letting you view existing ones. Three apps already have sign-in buttons wired and tested; Google just went live after enabling it in Supabase via the Management API. The consent screen is still branded epiphany, so that'll need a rebrand in Google Cloud first.
