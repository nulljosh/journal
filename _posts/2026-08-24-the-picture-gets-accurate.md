...
Spam wave hit the account: six apps flagged for batch-submitting, appeals filed for the three strongest plus Healstack. Fixed Epiphany's native sign-in error by pulling the button, rewrote Lexly's listing to reduce fingerprinting, lightened Curvely's overlay. Renamed Wiretext to Charwork and restyled the iOS app with warm tones and rounded corners. Sidewise (Newsline) finished with age rating set and screenshots shipped, but held submission because seven apps remain rejected under the 4.3(a) spam wave. Changed NYC's menu to show a live demo game running behind a scrim instead of the static skyline, with demoMode hard-gating it so real games stay player-controlled; clicking NEW GAME tweens the camera and exits the demo cleanly.
