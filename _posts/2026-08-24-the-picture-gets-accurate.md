...
Shipped Inkpress' first-ever Mac release alongside iOS submission, added entry thumbnails, and built headless screenshot tooling. Re-shot Healstack screenshots, pushed NYC to ten App Store screenshots with promo text, audited GTM ledger (real blocker: CRA tax form), and diagnosed flights map as Cloudflare egress IP block. Read Lexly's 4.3(a) appeal rejection via Chrome (ASC still down): Apple denied the appeal, citing spam similarity, so I cancelled the iOS submission and repositioned the entire app around BC Grade 12 school curriculum (Pre-Calc, AP Bio, Anatomy) instead of generic language learning. Rewrote listings, reordered the app to open on school courses first, and deployed changes live. Lexly iOS stays live on the previous version; macOS remains in review.
