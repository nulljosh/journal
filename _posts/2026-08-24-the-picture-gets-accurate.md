...
Spam wave hit six apps with 4.3(a) flags; filed appeals for three plus Healstack. Fixed Epiphany's sign-in error, renamed the Newsline project to Sidewise (repo, Worker, Xcode targets, all docs updated), renamed Wiretext to Charwork, and resubmitted both Sidewise platforms to review at Joshua's direction. Audited the portfolio for stale names, including the Doorstock landing page which had been a dead link; redeployed after fixing. Found the real problem in the evening: a .gitignore had excluded entire directories, so committed untracked source from credis, scripts, agent-101, and roost, then cleaned up the tree, refreshed architecture diagrams across twenty-eight repos, pruned roadmap items, and corrected stale names in the wiki. Landing pages got animation across the portfolio: NYC shows live attract-mode gameplay, Litigate has the house vibe with bulb-block hero and docket animation, Healstack cycles glyphs, Curvely's curve wall visible, Voxprint swapped to neutral ink, all on Cloudflare Pages and now live at litigate.heyitsmejosh.com with leak tests passing.
