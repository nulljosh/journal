...
Got Lexly running on iOS: the catalog was breaking because it had been invisible on web, so I moved the masterclasses to bookrank and fixed it. Reworked units from keywords instead of tiers, added lesson cards with grammar notes across five languages, and loosened the filter to boost smaller languages. Audited the whole portfolio (32 apps, 12 live) and found Form 506 gates iOS revenue (mailed+faxed request, not online), no analytics dashboards, and no canonical metadata. Repriced Voxprint to $9.99 and swept support/marketing/privacy URLs across all apps: Litigate and Bookrank point at dead hosts since their renames, Lexly's 404s at the portfolio root, and both Litigate/Bookrank are frozen READY_FOR_SALE until next release since app-info is only editable during staged versions. Web Stripe works; missing is CTAs and products, not infrastructure.
