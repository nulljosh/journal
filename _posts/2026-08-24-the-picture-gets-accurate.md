...
Spam wave hit six apps with 4.3(a) flags; filed appeals for three plus Healstack. Fixed Epiphany's sign-in error, renamed Wiretext to Charwork, resubmitted Sidewise (availability set, test suite green, 0 errors/0 blocking) to review at Joshua's direction despite the risk. Portfolio audit caught four apps with stale names on the Work list; updated them to Quotestreak, Doorstock, Sidewise, and Curvely's live site. Found the real problem in the evening: a catch-all .gitignore had been silently excluding entire live directories, so committed untracked source from credis, scripts, agent-101, and roost, then deleted stale feature audits and moved backups out of the repo tree. Landing pages got their animation: NYC shows live attract-mode gameplay, Litigate has the house vibe with an animated bulb-block hero and staggering docket card, Healstack cycles eight health glyphs, Curvely's curve wall now visible, and Voxprint swapped from the portfolio's yellow to neutral ink, all with CSS hero animations canonicalized to their own domains, all pure CSS on Cloudflare Pages.
