...
Pulled canonical App Store metadata from live and dropped China from four apps to dodge 2.1 rejections. Charwork and Curvely shipped macOS versions with shared HTTP APIs and MCP servers ported from Sidewise, fixing entitlements and icon sizing. Chased an analytics gap that turned out not to exist: Cloudflare Web Analytics has been live six months via edge injection (no visible script tag, which is why grepping found nothing). Verified the dashboard shows real traffic across subdomains. Grateful I checked before manually wiring beacons that would have double-counted.
