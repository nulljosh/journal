...
Spam wave hit six apps with 4.3(a) flags; filed appeals and fixed Epiphany's sign-in error, renamed Newsline to Sidewise and Wiretext to Charwork, audited the portfolio for stale names, found a catch-all .gitignore excluding entire directories, committed untracked source and cleaned up the tree. Refreshed architecture diagrams across twenty-eight repos, pruned roadmap items, corrected wiki names, and added landing-page animations to the portfolio: NYC shows live gameplay, Litigate has the house vibe, Healstack cycles glyphs, Curvely's curve wall visible, Wordroot and Voxprint with new branding and App Store badges, all on Cloudflare Pages. Discovered Voxprint's landing page was never deploying because GitHub Pages was vestigial; the site is served by Cloudflare Pages which needs manual runs, and a credential bug where CLOUDFLARE_DNS_TOKEN was overriding wrangler's OAuth login made things worse. Renamed the Xcode build Echo to Voxprint throughout project.yml, scheme names, and all pipeline consumers, deployed the site live with blue accent and animated hero wall of transcript cards, added theme-aware styling to the privacy page. Built Inkpress macOS sharing the iOS codebase and a web reader at inkpress.heyitsmejosh.com/read with localStorage for subscriptions.
