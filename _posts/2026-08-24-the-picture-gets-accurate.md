...
Built a dream journal that interprets entries against previous ones, deployed to Cloudflare Workers with a landing page hero of animated lights. Safety testing found it was matching dates instead of content, so added deterministic checks before model calls. Wired a Lexly CI workflow awaiting a Cloudflare API token.
