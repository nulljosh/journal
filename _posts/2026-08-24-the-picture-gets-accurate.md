...
Fixed Talli's redirect loop, shipped PWA across twenty apps, and built Swing (random video chat with Durable Object pairing), then polished its landing page with rewritten hero copy and a warmer accent color. Closed a critical Homeward security hole where edit tokens leaked from the read policy, fixed by moving them to a create-listing RPC with landing page, icon, and OAuth sign-in. Fixed the journal's service worker (was cache-first, now network-first with offline fallback), which had been serving stale HTML and CSS to anyone who had visited before, making some fixes look like they never landed.
