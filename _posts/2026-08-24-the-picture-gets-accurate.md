...
Fixed Talli's redirect loop, shipped PWA across twenty apps, and built Swing (random video chat with Durable Object pairing). Closed a critical Homeward security hole where edit tokens leaked from the read policy, fixed by moving them to a create-listing RPC with landing page, icon, and OAuth sign-in. Fixed the journal's CSP style-src (which had been refusing color tokens and animations) and a service-worker bug pinning stale builds; rewrote the generator, rebuilt the landing page with house hero style, consolidated domains (pets redirects to homeward), fixed the manifest, and seeded three lost-cat posters from Langley, the same CSP bug found and fixed on sidewise.
