...
Spam wave hit (six apps flagged, three appeals filed plus Healstack), and fixes landed: Epiphany's sign-in, Lexly's listing fingerprinting, Curvely's overlay. Renamed Wiretext to Charwork, restyled the iOS app with warm paper theme. Charwork's web site went live at charwork.heyitsmejosh.com (second domain on the wiretext Pages project); the old domain stays live because App Store URLs point to it. Restyled the web to match the iOS restyle (overriding Jaybulb tokens in src/index.css without touching the shared file), fixed UI bugs (line-height, button hierarchy, canvas gutter, palette readability), 26 tests pass. Sidewise availability set to 174 territories minus China, test suite shipped, iOS validated and held pending 4.3(a).
