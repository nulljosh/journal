...
Spam wave hit six apps with 4.3(a) flags; filed appeals for three plus Healstack, then fixed Epiphany's sign-in error, renamed Newsline to Sidewise and Wiretext to Charwork, and resubmitted both Sidewise platforms. Audited the portfolio for stale names (Doorstock had been a dead link; fixed and redeployed), then found the real problem: a catch-all .gitignore had excluded entire directories, so committed untracked source from credis, scripts, agent-101, and roost, cleaned up the tree, refreshed architecture diagrams across twenty-eight repos, pruned roadmap items, and corrected stale wiki names. Landing pages got animation across the portfolio: NYC shows live attract-mode gameplay, Litigate has the house vibe with bulb-block hero and docket, Healstack cycles glyphs, Curvely's curve wall visible, Voxprint swapped to neutral ink, all on Cloudflare Pages and live at litigate.heyitsmejosh.com with leak tests passing. Wordroot finalized its infrastructure: added an official App Store badge to the landing page and migrated the Cloudflare Pages project from etyma to wordroot, with DNS repointed and verified at wordroot.heyitsmejosh.com. Built Inkpress macOS alongside the iOS reader, sharing the codebase with three platform-specific call sites (nav bar title mode, text field modifiers, and file-open methods). Added a web-based reader client at inkpress.heyitsmejosh.com/read backed by browser localStorage for local feed subscriptions.
