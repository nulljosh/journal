...
Spam wave hit (six apps flagged, three appeals filed plus Healstack), and fixes landed: Epiphany's sign-in, Lexly's listing fingerprinting, Curvely's overlay. Renamed Wiretext to Charwork, restyled the iOS app with warm paper theme; the web site went live at charwork.heyitsmejosh.com with UI bug fixes (line-height, button hierarchy, canvas gutter), 26 tests passing. Sidewise availability set to 174 territories minus China, test suite shipped, iOS validated and held pending 4.3(a). Evening: audited the labs README and CLAUDE.md docs, found they listed deleted repos and missed six active ones; rewrote both to match reality, dropped the stale ship-state board, and deleted 91 lines of cruft.
