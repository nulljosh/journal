...
Discovered Lexly production was broken: CSP blocked Supabase SDK, site went blank. Fixed CSP, made auth graceful, gated catalog, dropped cache to 5 minutes, deduplicated content (punctuation/case-insensitive). Fixed iOS bug where exercises dropped half their fields; ported hearts and SM-2 to iOS, built 1,800 exercises from Tatoeba, added match/cloze (830→2,615). Beefed landing page: stats bar (2,600 exercises, 12 languages, 44 courses), five types, Tatoeba, FAQ; built playable demo (/app/?demo=1, no signup, no-op save guard); added OG/Twitter cards, dark mode, WebP (347KB→79KB), a11y. Verified new types via jsdom (14 assertions), fixed two layout bugs (image height, grid overlap), patched XSS in Sidewise, rolled security headers, fixed Talli's lock.
