...
Notes inbox flagged six apps for spam from batch-submitting the same day; filed appeals for the three strongest and added Healstack as a sixth. Epiphany's rejection was readable once I checked Resolution Center: Apple's native sign-in sheet was throwing an error. Pulled that button and left only email/password. Investigated Lexly's 4.3(a) rejection and rewrote the listing to be more unique (tagline + icon fingerprinting were matching competitor apps too closely), added a boot splash, closed a Resend leak, and filed an appeal explaining the changes. Fixed Curvely's landing page (the dark scrim overlay was obscuring the animated curve-card background), lightening it to fade just over the headline.
