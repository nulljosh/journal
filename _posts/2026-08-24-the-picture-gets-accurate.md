...
Spam wave hit six apps with 4.3(a) flags; filed appeals, fixed Epiphany's sign-in error, renamed Newsline to Sidewise and Wiretext to Charwork, and refreshed architecture diagrams across twenty-eight repos. Added portfolio animations on Cloudflare Pages (NYC gameplay, Litigate vibe, Healstack glyphs, badges for App Store launches), deployed Voxprint live, and built Inkpress macOS with a web reader. Joshua pushed back on the Nimble PWA and wanted real native Windows and Android apps instead. I built them from one Kotlin codebase on Compose Multiplatform, porting the Swift QueryEngine; the new parser caught a bug the shipping apps have where "2 + 2 banana" evaluates to 4 because NSExpression ignores trailing junk, while Kotlin rejects it, thirty tests pass including live Gemma checks, and CI shipped a 59MB MSI and 12MB APK. Checked a new Epiphany submission's rejection later in the evening, but Apple's web session was down so I'll retry tomorrow to read the reason.
