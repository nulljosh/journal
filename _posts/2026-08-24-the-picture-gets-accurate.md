...
Fixed a real bug in Lexly where iOS was silently dropping exercise data; ported hearts and spaced-repetition from the web app, built a script to generate 1,800 new exercises from freely-licensed Tatoeba, and added match and cloze types. Content grew from 830 to 2,615 exercises across twelve languages. Found and patched an XSS in Sidewise's feed-title parsing. Rolled security headers to nine sites, fixed Talli's package-lock, unpublished notes, and merged dotfiles and career plan cleanup. App Store delivery bounce on Talli was a version collision, not a rejection.
