...
Got Lexly running on iOS: the catalog was breaking because it had been invisible on web, so I moved the masterclasses to bookrank and fixed it. Exercise types work, hearts decrement properly. Reworked units from difficulty tiers to real content matched against keywords, so "Food & Drink" actually contains food. Added a lesson card to each course with three real phrases and a grammar note: wrote fifty tips across Spanish, French, German, Italian and Portuguese. Loosened the sentence filter to allow one unknown word, which jumped smaller languages: Hindi went 6.6x from 284 to 1,863 usable pairs, and all twelve languages hit ten units for the first time.
