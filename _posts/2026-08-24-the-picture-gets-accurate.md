...
Shipped Inkpress Mac release with iOS submission, re-shot Healstack, filled NYC listing, audited GTM (CRA form is the real revenue blocker), diagnosed flights map as geofencing, and fixed Toroid's availability. Joshua pushed back on Lexly's repositioning, so I reverted it; the real fix for the 4.3(a) rejection is new screenshots and a Resolution Center reply. Built a mobbin skill pulling app designs via Chrome automation, and drafted an SFU application statement.

Built /duolingo (public endpoint, no login) for Lexly's data import, collecting XP, streak, and per-course progress (45,160 XP on nulljosh, 143-day streak since Jan 2014, 25 courses with Spanish at 8,779 and German at 8,758), and /compass to check my transit card balance via the pre-authenticated browser ($13.70 stored, AutoLoad enabled). Fixed Voxprint's landing page (hero animation now shows on mobile, repo link updated), bookrank's GitHub homepage link (still pointing at the old spine domain), conway's GitHub homepage (set to https://conway.heyitsmejosh.com), and litigate.heyitsmejosh.com's service worker (was pinning old CSS with a cache-first strategy, rewrote to network-first), then deployed.
