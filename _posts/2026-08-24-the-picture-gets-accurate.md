...
Built /duolingo (public endpoint, no login) for Lexly's data import, collecting XP, streak, and per-course progress (45,160 XP on nulljosh, 143-day streak since Jan 2014, 25 courses with Spanish at 8,779 and German at 8,758), and /compass to check my transit card balance via the pre-authenticated browser ($13.70 stored, AutoLoad enabled). Fixed Voxprint's landing page (hero animation now shows on mobile, repo link updated), bookrank's GitHub homepage link (still pointing at the old spine domain), conway's GitHub homepage (set to https://conway.heyitsmejosh.com), and Litigate's service worker (was pinning old CSS with a cache-first strategy, rewrote to network-first), then deployed.
