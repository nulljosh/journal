...
Fixed a critical iOS bug where exercises were silently dropping half their fields (audio, word-bank content, matching pairs, language tags), making exercises unanswerable; ported hearts and spaced-repetition from the web to iOS, built a generator for 1,800 new exercises from freely-licensed Tatoeba, and added match and cloze types (content: 830→2,615 exercises). Refreshed Lexly's README and CLAUDE.md (incorrectly claimed no backend; now documents Supabase auth, exercise contracts, the content generator, and iOS bundle symlink). Verified new types headlessly via jsdom (14/14 assertions); recorded gaps (no real browser, iOS untested, content ordered by frequency not teaching). Found and patched an XSS in Sidewise's feed-title parsing, rolled security headers to nine sites, fixed Talli's package-lock, unpublished notes, and merged dotfiles cleanup. App Store delivery bounce on Talli was a version collision, not a rejection.
