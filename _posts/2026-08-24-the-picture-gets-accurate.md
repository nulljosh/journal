...
Audited auth across the whole codebase and shipped GitHub sign-in to Litigate, Lexly, and Bookrank. GitHub works; Apple needs credentials not yet configured in Supabase. Resend's unverified domain blocks password reset. Wired a check that probes the authorize endpoint, so broken buttons can't ship again.

Refreshed Epiphany's App Store screenshots with real data and fixed three test infrastructure bugs. Found the portfolio balance not matching the holdings list. Updated BCGD's icon palette and sitemap.

Spent the evening auditing Vancouver Frenchies, a French Bulldog breeder where I bought a puppy years ago. Their Wix site has three concrete problems: the Available Puppies page is broken (301-redirect to home), the homepage is 20.6 megabytes, and the application is a PDF with offline payment. Pitched them a redesign: static site on Cloudflare Pages, an apply-form Worker, $2,500 build plus $150 a month to host.
