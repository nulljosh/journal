...
Shipped Inkpress' first-ever Mac release alongside the iOS submission, added entry thumbnails across platforms, and built a headless screenshot capture tool for App Store reviews. Re-shot Healstack's iOS screenshots (only two frames broken) and fixed the tab-bar overlap, finding a new clipping bug. Pushed NYC's App Store listing to ten screenshots plus promotional text, re-verified the GTM ledger (six stale rows), and diagnosed the flights map as a Cloudflare egress block with no escape hatch. Found the real reason none of the apps can charge money: a Canadian GST/HST Form 506 waiting on a number CRA refused to read over the phone and is mailing instead. Revenue unblocks once it arrives.

Finished Bookrank's Phase C with Supabase auth, private summaries, account deletion, and read-aloud playback on the summary cards. Fixed Charwork's build pipeline which had been wiping dist on every rebuild. Backfilled Windows and Android apps for Nimble by porting the Swift math parser to Kotlin and wiring CI to ship updates automatically. Fixed npm test mode in Epiphany and Sparkjar (they were entering watch mode instead of exiting). Pruned completed roadmap items across six repos.
