...
Got Lexly running on iOS: the catalog was breaking because it had been invisible on web, so I moved the masterclasses to bookrank and fixed it. Reworked units from keywords instead of tiers, added lesson cards with grammar notes across five languages, and loosened the filter to boost smaller languages. Audited the whole portfolio (32 apps, 12 live) and found three blockers: GST/HST Form 506 gates all iOS revenue, no analytics dashboards exist, and no metadata is checked in. Fixed Voxprint's IAP price from $7.99 to $12.99 and found Talli's support URL dead since the rename; both fixes wait for the next version bump.
