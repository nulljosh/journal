...
Spam wave hit: six apps flagged, appeals filed for three plus Healstack, and fixed Epiphany's sign-in error, Lexly's listing fingerprinting, Curvely's overlay, renamed Wiretext to Charwork and restyled the iOS app. Sidewise availability set via dashboard (174 territories minus China), test suite shipped (95 checks, fixed JSON-RPC null params bug), iOS 1.0 validated at 0 errors/0 blocking and submit-ready, held pending 4.3(a) responses, macOS 1.0 needs screenshots attached. Charwork's web site went live at charwork.heyitsmejosh.com (second custom domain on the wiretext Pages project); wiretext.heyitsmejosh.com stays live because App Store URLs point at it. Restyled the web to warm paper to match the iOS restyle, overriding Jaybulb tokens in src/index.css without touching the shared file; fixed real UI bugs (line-height borders, toolbar button hierarchy, canvas gutter, palette readability). 26 vitest tests pass.

Evening work: audited the two root docs in labs, found README listed repos that no longer exist and missed six that it actually tracks. Rewrote both README and CLAUDE.md to be accurate - the codebase map now matches what's on disk, nine undocumented repos are now listed, and the stale ship-state board is gone (asc versions list is the source of truth). Deleted 91 lines of cruft and committed it all together.
