...
Shipped Inkpress' first-ever Mac release alongside the iOS submission, added entry thumbnails across all platforms, and built a headless screenshot capture tool for App Store reviews. Re-shot Healstack's iOS screenshots (only two frames were broken, not the whole set) and fixed the tab-bar overlap by launching directly to each screen, finding a new clipping bug in the process. Pushed NYC's App Store listing from four screenshots to ten plus promotional text, and re-verified the GTM ledger against live data, finding six stale rows. The flights map is a Cloudflare egress block affecting every free ADS-B source, so the Vercel escape hatch is dead. Read the Paid Apps Agreement status from App Store Connect: banking is processing normally, US tax forms are active, the blocker is Canadian GST/HST Form 506, and one call to Apple determines whether it accepts a not-registered declaration (which would unblock revenue immediately).
