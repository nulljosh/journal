...
Fixed Lexly's iOS catalog, repriced Voxprint, swept App Store URLs, and finished Bookrank's README with headless screenshots. Swept security headers across eight web projects and rebuilt Doorstock and Feng Shui landing pages to house style. Deleted the privacy site and fixed Talli's empty-dashboard bug by raising URLSession timeouts from 15 to 60 seconds. Started a Craigslist classifier tool: their site talks to an undocumented JSON API that requires no login, returns search results as bare number arrays. Reverse-engineered the format (posting ID and date as base offsets, neighborhood indexed from coordinates), built a decoder and test against real data, and wrote a Cloudflare Worker serving clean results.
