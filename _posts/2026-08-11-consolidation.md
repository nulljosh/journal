...
Evening: swapped the portfolio to a Fez-inspired theme that replaces the Geist pastels with parchment tones on light (#f6ead0) and deep blue (#141f33) on dark, burnt-orange and gold for link hovers. Built a new tokens-fez.css with square corners and a pixelated memoji avatar (deliberate pixel art, no webfont). Updated the theme-color meta tags to track the new palette. Pushed.

**Apps.** Updated roadmap and wiki status across Curvely, NYC, Wiretext, Healstack, Sparkjar, Lexly, Talli, Litigate, and BCGD. Consolidated what was known into a real Ship Status that the live API now confirms. Healstack build now VALID on ASC. Portfolio restyled. No new features shipped, but the plan is much clearer now.
