...
Fixed Talli's header avatar cold-start blocking: `AppState.init()` was doing a synchronous CG draw to generate the node-graph avatar on cache miss (first launch or missing disk cache), freezing the main thread for hundreds of milliseconds and showing a blank screen before the letter placeholder could even render. Moved the generation into a background `Task`, so now the letter placeholder appears instantly and the full avatar swaps in once generated — feels instant on startup.
...
Late-night fix on the portfolio: the Animoji avatar was showing a visible white halo ring against dark backgrounds, caused by anti-aliased fringe pixels baked into the image exports. Fixed by eroding the alpha channel to strip the fringe, verified clean in both light and dark mode previews, and pushed live to heyitsmejosh.com.

Lexly got a visual fix tonight: the Fraunces serif font on the landing page and app headers was making everything look cursive/wrong, so switched back to the same sans-serif stack as the body text. Also fixed a redirect bug where the two masterclass content cards (Pre-Calc 12 and AP Biology) were bouncing back to the landing page instead of opening — turned out the catalog.json URLs were relative paths that resolved wrong from the `/app/` route, made them absolute. Brain-dumped a stack of UI/feature requests: nav bar clutter, icon color issues, footer spacing, more content, avatar refresh feature (like Epiphany has), masterclass prominence, and iOS/macOS parity gaps (iOS is missing the masterclass reader and avatar picker that web already has). Logged everything to the roadmap for next session's triage.
